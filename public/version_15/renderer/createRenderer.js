...
export default function createRenderer(options) {
  ...
  function mountComponent(vnode, container, anchor) {
    ...
    if (isFunctional) {
      // 如果是函数式组件 则将vnode.type作为渲染器函数，将vnode.type.props作为props选项自定义即可
      componentOptions = {
        render: vnode.type,
    ...
    // get props and attrs
    const [props, attrs] = resolveProps(propsOption, vnode.shouldKeepAlive ? propsOption : vnode.props);
    ...
    const instance = {
      ...
      mounted: [],
      // 只有KeepAlive组件的实例下才会有keepAliveCtx属性
      keepAliveCtx: null,
    };

    // 检查当前要挂载的组件是否是keepAlive组件
    const isKeepAlive = vnode.type.__isKeepAlive;
    const isTransition = vnode.type.__isTransition;
    if (isKeepAlive) {
    ...
    const setupContext = { attrs, emit, slot };
  ...
  function patch(n1, n2, container, anchor) {
    ...
    } else if (typeof type === 'object' && type.__isTransition) {
